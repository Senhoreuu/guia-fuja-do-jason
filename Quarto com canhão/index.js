...
const getDistance = (user, target, accept) => {
  const x1 = user.getX();
  const y1 = user.getY();
  const x2 = target ? target.getX() : 12;
  const y2 = target ? target.getY() : 8;

  return accept
    ? Math.abs(Math.floor(Math.hypot(x1 - x2, y1 - y2)))
    : Math.abs(Math.hypot(x1 - x2, y1 - y2));
};

const getAround = (user, furni) => {
  const rotation = furni.getR();
  const players = [];
  const coords = { x: furni.getX(), y: furni.getY() };

  switch (rotation) {
    case 0:
      players.push(
        ...Room.getEntitiesByCoord(coords.x - 1, coords.y),
        ...Room.getEntitiesByCoord(coords.x - 2, coords.y)
      );
      break;

    case 2:
      players.push(
        ...Room.getEntitiesByCoord(coords.x, coords.y - 1),
        ...Room.getEntitiesByCoord(coords.x, coords.y - 2)
      );
      break;
  ...
  const userId = room.users_info[user].id;

  players.forEach((player) => {
    if (player.getEffect() === 4) return;
    player.setEffect(4);

    const targetId = room.users_info[player].id;
    const targetName = room.users_info[player].name;

    if (userName === targetName) {
      player.gotoRoom(924213);
      Events.sendMessageToRoom(924213, 'kys', JSON.stringify({ user: targetName }));
    } else {
      Highscores.add(user, 1);
      Events.sendMessageToRoom(924213, 'cannon', JSON.stringify({ user: userName, target: targetName, id: userId, targetId }));
    }

    player.notification('WEEN', 'Você foi eliminado(a) por um canhão!');
    player.gotoRoom(924213);
  });
};

const cannon = (user, furni) => {
  if (furni.getSprite() !== 4922) return;

  const id = user.getId();
  const effect = user.getEffect();

  if (game.waitCannon.has(id) || effect !== 0) return;

  const distance = getDistance(user, furni, true);

  if (distance >= 3) return;

  furni.toggleState();

  getAround(user, furni);

  game.waitCannon.add(id);
  wait(() => game.waitCannon.delete(id), 3);
};

Events.on('interact', cannon);
